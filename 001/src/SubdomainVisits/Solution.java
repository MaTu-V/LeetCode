...
public class Solution {
    public void  getSub(int count,String str,HashMap<String,Integer> map) {
        if (map.containsKey(str)){
            map.put(str,map.get(str) + count);
        }else {
            map.put(str, count);
        }
        int cur = str.indexOf(".");// 下次出现位置
        while (cur > 0) {
            String key = str.substring(cur + 1);
            if (map.containsKey(key)){
                map.put(key,map.get(key) + count);
            }else {
                map.put(key,count);
            }
            cur = str.indexOf(".", cur + 1);
        }
    }
    ...
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (int i = 0; i < cpdomains.length; i++) {
            ...
            getSub(Integer.parseInt(sp[0]),sp[1],map);
        }
